 ...
 * It takes in two parameters: the secret code array and an array of colors chosen by the user
 ...
 */
function countBlacks(secret, guess) { // The generated secret code and the users guess will be passed as argument into this function
    let blackCounter = 0;
 ...
 * Firstly, this function gets rid of the correct color guesses which are in the right position
 * by creating arrays secretWithoutBlacks and guessWithoutBlacks
 * Secondly, it checks if colors in the user's guess are included in the secretWithoutBlacks.
 * The include function is sufficient here, as we no longer worry about the position of the colors.
 */
function countWhites(secret, guess) {
    ...
    for (let i = 0; i < secret.length; i++) {
        if (secret[i] !== guess[i]) { // If elements on a certain index position in secret code and the user's guess are not identical 
            secretWithoutBlacks.push(secret[i]); // we add the element to an array secretWithoutBlacks
            guessWithoutBlacks.push(guess[i]); // we also add the element to an array guessWithoutBlacks
        }
    }

    //Now we have two arrays without the guessed colors with corresponding index position
    while (guessWithoutBlacks.length > 0) {
        if (secretWithoutBlacks.includes(guessWithoutBlacks[0])) { // includes() tells us if the element in one array is included in the other array
